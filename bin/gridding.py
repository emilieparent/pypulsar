...
def fit(init_params, data):
    ...
    def errorfunction(p): return np.ravel(observed_snr(*p)(ras, decs) - snrs)
    if debug:
        output = opt.leastsq(errorfunction, init_params,
...
def main():
    ...
    if debug:
        ...
        print("\tSNR:", init_params[0], "RA:",
              init_params[1], "Dec:", init_params[2])
    global beam_profile
    # Use gain = 1
    ...
    plt.scatter((ras-psrra)*60/15.0, (decs-psrdec)*60,
                c=snrs, marker='o', label='_nolegend_')
    plt.spring()
    cbar = plt.colorbar()
    ...
    plt.subplot(212)
    plt.plot(angseps, psrsnr *
             beam_profile.gain_at_angular_offset(angseps), 'k', zorder=-1)

...
def kepler_function(asini, p_orb, p_psr, T0, ecc=0, peri=0):
    ...
    def func(mjd):
        ...
        A = between_zero_twopi(
            2*np.arctan(np.sqrt((1+ecc)/(1-ecc))*np.tan(E/2.0)))  # True anomaly

        velocity = orb_freq_hz*asini / \
            np.sqrt(1-ecc**2)*(np.cos(peri+A)+ecc *
                               np.cos(peri))  # in units of C
...
def min_comp_mass(Pb, x):
    ...
    # inclination angle in rad (assume edge on for min companion mass)
    i = psr_utils.PIBYTWO

    def f(mc): return psr_utils.mass_funct(Pb*psr_utils.SECPERDAY,
                                           np.fabs(x))-psr_utils.mass_funct2(mp, mc, i)
...
def fit(params, data):
    ...
    """
    ps, perrs, mjds = data
    def errorfunction(p, vals, errs): return np.ravel(
        (kepler_function(*p)(mjds) - vals)/errs)
    if options.debuglevel > 0:
        p, cov_x, infodict, mesg, success = opt.leastsq(
            errorfunction, params, maxfev=options.maxfev, full_output=1, args=(ps, perrs))
        print("Covariance matrix:\n", cov_x)
    else:
        p, success = opt.leastsq(errorfunction, tuple(
            params), maxfev=options.maxfev, args=(ps, perrs))
...
def main():
    ...
    t_actual = np.linspace(
        mjds.min()-0.5*result[1], mjds.max()+0.5*result[1], mjds.ptp()*1000)
    ...
    if options.debuglevel > 0:
        # plot initial guess
        initphases = np.fmod(
            mjds-init_params[3], init_params[1])/init_params[1]
        initphases[initphases < 0] = initphases+1
        plt.plot(phasegrid, kepler_function(*init_params)
                 (init_params[3]+phasegrid*init_params[1])-init_params[2], 'r--')
...
if __name__ == '__main__':
    ...
    parser.add_option('--use-pfds', dest='usepfds', action='store_true',
                      help="Files provided contain a list of pfd files to read. (This is the default)", default=True)
    parser.add_option('--use-text', dest='usepfds', action='store_false',
                      help="Files provided are text files containing 3 columns: mjd, period, period error. (Default is files contain list of pfd files to read)", default=True)
    parser.add_option('-f', '--file', dest='filename', type='string',
                      help="File containing a list of pfd files to use, or a list of mjds and periods.", default='')
    parser.add_option('-d', '--debug', dest='debuglevel', type='int',
                      help="Set the debug level. Higher level means more information will be printed to screen. (Default: No debug info", default=0)
    parser.add_option('-v', '--verbose', dest='verbose', action='store_true',
                      help="Be verbose. Same as --debug=1. (Default: Not verbose)", default=False)
    parser.add_option('-x', '--extend', dest='extendnum', type='int',
                      help="If barycentric p-dot is significant, estimate period just before and after measured value and include the estimated values in the fit. The argument should be the maximum number of estimated values to use on either side of the measured period. (Default: 0)", default=0)
    parser.add_option('-e', '--efac', dest='efac', type='float',
                      help="Factor to multiply all uncertainties by. (Default: 1)", default=1)
    parser.add_option('--maxfev', dest='maxfev', type='int',
                      help="Maximum number of function evaluations in fitting. (Default: 1000)", default=1000)
    parser.add_option('-s', '--savefn', dest='savefn', type='string',
                      help="Filename to save plot to. (Default: 'fitkepler.tmp.ps')", default='fitkepler.tmp.ps')
    parser.add_option('-n', '--noplot', dest='plot', action='store_false',
                      help="Do not show plot. Only make ps file. (Default: Show plot)", default=True)
    parser.add_option('-i', '--init-params', dest='init_params', type='string',
                      help="Initial parameters separated by commas (no spaces!) Order is: Asini (in lt-s), Orbital period (in days), Pulsar period (in s), T0 (in MJD), Eccentricity, Logitude of periastron (in rad). Example: '5,6.25,0.004909,54905,0.67,-1.51' (NOTE: excluding last two params will result in fitting for a circular orbit.)")
    parser.add_option('-p', '--predict', dest='predict_mjds', type='string',
                      help="MJDs separated by commas at which to predict the spin period (no spaces!)")
    options, args = parser.parse_args()

    if options.verbose:
        options.debuglevel = 1
    if options.init_params:
        init_params = np.array([float(x)
                               for x in options.init_params.split(',')])
    else:
        sys.stderr.write(
            "-i, --init-guess option MUST be provided! Exiting!\n")
        sys.exit(1)
    if options.predict_mjds:
        predict_mjds = np.array([float(x)
                                for x in options.predict_mjds.split(',')])
    else:
        predict_mjds = []

...
def main():
    ...
    reqendsamp = reqendsamp - (reqendsamp %
                               options.downsamp) + options.downsamp
    # Get extra bins for smoothing
    ...
    if options.dm:
        ...
        maxsamps = int(np.round(float(maxsamps)/options.downsamp)
                       )*options.downsamp
        endsamp += maxsamps
    ...
    if options.width > 1:
        ...
        data = smooth(data, factor=options.width)[
    ...
    if options.dm is not None:
        ...
        dedisp_prof = dedisperse(data, delay_samples)[
            :-maxsamps/options.downsamp]
        plt.plot(np.linspace(xlim[0], xlim[1],
                 dedisp_prof.size), dedisp_prof, 'k-')

...
class FreqInterval:
    ...
    def __contains__(self, other):
        if type(self) != type(other):
            raise ValueError(
                "Contains test must be made between two FreqInterval objects.")
    ...
    def __add__(self, other):
        if type(self) != type(other):
            raise ValueError(
                "Addition must be between two FreqInterval objects.")
...
def main():
    ...
    plt.errorbar(freqs, filenums, xerr=freqerrs,
                 fmt=None, zorder=1, ecolor='k')
    ...
    plt.hist(filenums, bins=max(filenums), range=(
        0, max(filenums)), orientation='horizontal', fc='none')

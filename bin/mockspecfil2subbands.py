...
parser = optparse.OptionParser(
    usage='%prog [options] infile', description="Convert filterbank data (from MockSpec data) to PRESTO subbands. Each subband is one channel.")
parser.add_option('-o', '--outname', dest='outname', type='string',
                  help="Output filename. Do not include extension.", default=None)
...
if options.outname is None:
    sys.stderr.write(
        "An outname must be provided. (Use -o/--outname on commandline).\n")
    sys.exit(1)
...
def writeInfoFile(filfile):
    ...
    inffile.write(
        " Data analyzed by                       =  Patrick Lazarus\n")

    inffile.write(" Any additional notes:\n")
    inffile.write(
        "    Input filterbank file created from MockSpec data (AO)\n")
    inffile.write("    using psrfits2fil, written by Julia Deneva (?)\n")
    inffile.write(
        "    Subbands and inf file created by mockspecfil2subbands.py\n")
...
def main():
    ...
    if infbfile.header['foff'] > 0:
        filenames = ["%s.sub%04d" % (options.outname, subnum)
                     for subnum in range(0, infbfile.header['nchans'])]
    elif infbfile.header['foff'] < 0:
        # Invert subband files with respect to filterbank file.
        filenames = ["%s.sub%04d" % (options.outname, subnum) for subnum in range(
            infbfile.header['nchans']-1, 0-1, -1)]
    else:

...
def calc_percentile(pffts, percent=50):
    ...
    sys.stdout.write(
        "Calculating percentile score (%.1f %%)... %5.1f %%" % (percent, 0))
    sys.stdout.flush()
...
def plot(freqs, spectrum, style="k-", **plotargs):
    ...
    # 0.125 Hz is min freq searched in PALFA
    subones = (freqs >= 0.125) & (freqs < 1)
...
def gen_mask(freqs, powerspec, nsig=3.5):
    ...
    halfflat.sort()
    def cdfresids(sigma): return scipy.stats.norm(loc=0, scale=sigma).cdf(
        halfflat)-(np.arange(1, (halfflat.size+1))/(halfflat.size*2.0))
    sys.stdout.write("Optimizing... ")
    sys.stdout.flush()
    fitresult = scipy.optimize.leastsq(
        cdfresids, np.abs(np.array([halfflat[halfflat.size/2]])))
    sigma = fitresult[0]
    print("Done")
...
def hone_mask(freqs, powerspec, inmask, nsig):
    ...
    for block in np.arange(0, powerspec.size, BLOCKSIZE):
        ...
        smoothed_block = smooth(detrended_block, SMOOTHFACTOR)[
            lo:detrended_block.size-hi]
        outmask[block:blockend] = (smoothed_block > (std_block*nsig))
        sys.stdout.write("\rImproving mask... %5.1f %%" %
                         (100.0*blockend/powerspec.size))
        sys.stdout.flush()
...
def write_zaplist(zapfn, freqs, mask):
    ...
    zapfile.write(
        "# Lines beginning with 'B' are barycentric freqs (i.e. PSR freqs)\n")

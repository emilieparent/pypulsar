...
def pbdot(pulsar_mass, companion_mass, pb, ecc):
    ...
    """
    def f(e): return ((1 + (73.0/24)*e**2 + (37.0/96.0)*e**4)/((1-e**2)**(3.5)))
...
def main():
    ...
    cb.set_label(
        r"Time span needed to detect $\.P_b$ (with $\sigma$=%d; days)" % NSIG_DETECT)
    ...
    def display(mp, mc): return r"Mp=%g, Mc=%g (tspan=%d days, Pb-dot=%.3g s/s)" % \

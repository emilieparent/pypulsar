...
class PsrfitsFile(object):
    ...
    def get_spectra(self, startsamp, N):
        ...
        if trunc > 0:
            ...
            raise ValueError(
                "Number of bins to truncate is negative: %d" % trunc)
...
class SpectraInfo:
    def __init__(self, filenames):
        ...
        for ii, fn in enumerate(filenames):
            if not is_PSRFITS(fn):
                raise ValueError(
                    "File '%s' does not appear to be PSRFITS!" % fn)
            ...
            if MJDf < 0.0:
                raise ValueError(
                    "File %d seems to be from before file 0!" % ii)
            ...
            if 'DAT_FREQ' not in subint_hdu.columns.names:
                ...
                if ii == 0:
                    ...
                    if np.any((ftmp - self.df)) > 1e-7:
                        warnings.warn(
                            "Channel spacing changes in file %d!" % ii)
                else:
        ...
        if self.bits_per_sample < 8:
            ...
            self.bytes_per_spectra = (
                self.bits_per_sample * self.samples_per_spectra)/8
    ...
    def __str__(self):
        ...
        """
        result = [
        ]  # list of strings. Will be concatenated with newlines (\n).
        result.append("From the PSRFITS file '%s':" % self.filenames[0])
        result.append("                       HDUs = %s" %
                      ', '.join(self.hdu_names))
        ...
        mjdtmp = "%.14f" % fmjd
        result.append("  MJD start time (DATE-OBS) = %5d.%14s" %
                      (imjd, mjdtmp[2:]))
        result.append("     MJD start time (STT_*) = %19.14f" %
                      self.start_MJD[0])
        ...
        result.append("           Sample time (us) = %-17.15g" %
                      (self.dt * 1e6))
        ...
        if self.chan_dm != 0.0:
            result.append("   Orig Channel width (MHz) = %-17.15g" %
                          self.orig_df)
            result.append("    Orig Number of channels = %d" %
                          self.orig_num_chan)
            result.append("    DM used for chan dedisp = %-17.15g" %
                          self.chan_dm)
        result.append("      Total Bandwidth (MHz) = %-17.15g" % self.BW)
        result.append("         Spectra per subint = %d" %
                      self.spectra_per_subint)
        result.append("            Starting subint = %d" %
                      self.start_subint[0])
        result.append("           Subints per file = %d" % self.num_subint[0])
        result.append("           Spectra per file = %d" % self.num_spec[0])
        result.append("        Time per file (sec) = %-.12g" %
                      (self.num_spec[0]*self.dt))
        result.append("              FITS typecode = %s" % self.FITS_typecode)
        if debug:
            result.append("                DATA column = %d" % self.data_col)
            result.append("            bits per sample = %d" %
            ...
            result.append("        samples per spectra = %d" %
                          self.samples_per_spectra)
            result.append("           bytes per subint = %d" % subtmp)
            result.append("         samples per subint = %d" %
                          self.samples_per_subint)
            result.append("              Need scaling? = %s" % self.need_scale)
            result.append("              Need offsets? = %s" %
                          self.need_offset)
            result.append("              Need weights? = %s" %
                          self.need_weight)
            result.append("        Need band inverted? = %s" %
                          self.need_flipband)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Get info about PSRFITS files.")
